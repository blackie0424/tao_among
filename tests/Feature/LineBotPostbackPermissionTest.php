 ...
 *   - `start_batch_capture` → 後端須檢查 isEditor，viewer 應被拒絕
...
class LineBotPostbackPermissionTest extends TestCase
{
    ...
    public function test_viewer_cannot_start_batch_capture(): void
    {
        $fish = Fish::factory()->create();
        ...
        $this->invokeHandlePostback(
            $this->makePostbackEvent("action=start_batch_capture&fish_id={$fish->id}")
        );
    ...
    public function test_editor_can_start_batch_capture(): void
    {
        $fish = Fish::factory()->create();
        ...
        $this->assertCount(1, $replied);
        $this->assertInstanceOf(\LINE\Clients\MessagingApi\Model\TextMessage::class, $replied[0]);
        $this->assertStringContainsString('批次新增捕獲紀錄', $replied[0]->getText());
        $this->assertEquals(
            (string) $fish->id,
            Cache::get('line_user_' . self::USER_ID . '_batch_capture_fish')
        );
    }

    public function test_admin_can_start_batch_capture(): void
    {
        $fish = Fish::factory()->create();
