...
<div id="app" class="container mx-auto p-4" v-cloak>
    ...
    <div class="bg-white p-6 rounded shadow-md max-w-md mx-auto mb-6">
        ...
        <button type="button" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700"
                @click="uploadImage" :disabled="!selectedFile || uploading || imageUploaded">
            <span v-if="uploading">上傳中...</span>
            <span v-else-if="imageUploaded">已上傳</span>
            <span v-else>上傳圖片</span>
        </button>
        <div v-if="uploadError" class="text-red-600 mt-2" v-text="uploadError"></div>
        <div v-if="imageUploaded" class="text-green-600 mt-2">圖片已上傳，檔名：@{{ uploadedFileName }}</div>
    </div>
    ...
    <form @submit.prevent="submitFish" class="bg-white p-6 rounded shadow-md max-w-md mx-auto">
        ...
        <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded hover:bg-green-700"
                :disabled="!imageUploaded || submitting">
