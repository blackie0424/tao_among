...
const props = defineProps({
  ...
  fishNote: Object, // { id, note, note_type, locate }
})
...
onMounted(() => {
  // 初始化表單資料
  if (props.fishNote) {
    // 將 locate 首字母大寫以匹配 locates 的 value
    const locateValue = props.fishNote.locate.charAt(0).toUpperCase() + props.fishNote.locate.slice(1).toLowerCase()
    selectedLocate.value = locateValue
    selectedType.value = props.fishNote.note_type
    note.value = props.fishNote.note
  }
})
...
async function submitNote() {
  ...
  try {
    const res = await fetch(`/prefix/api/fish/${props.fish.id}/note/${props.fishNote.id}`, {
      method: 'PUT',
    ...
    if (!res.ok) throw new Error(data.message || '知識更新失敗')
    showSuccess.value = true
    goToFishDetail()
  } catch (e) {
    submitError.value = e.message || '知識更新失敗'
  } finally {
...
function goBack() {
  window.history.length > 1 ? window.history.back() : router.visit(`/fish/${props.fish.id}`)
}
</script>
