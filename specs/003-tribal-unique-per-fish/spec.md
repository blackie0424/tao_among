...
- FR-002: 介面僅顯示固定六個部落可選（值→顯示名：iraraley→朗島、ivalino→野銀、iranmeilek→東清、yayo→椰油、iratay→漁人、imorod→紅頭），不允許新增任意新部落名稱。
- FR-003: 若使用者嘗試新增已存在的「同一魚 × 同一部落」紀錄，系統必須拒絕並顯示建議：「請在備註補充不同意見」。
- FR-004: 備註欄位支援多段敘述與基本長度限制；提交時進行驗證與錯誤提示。
- FR-005: 對既有資料的重複部落採「僅標記重複待人工」策略（不自動合併），以不丟失資訊為原則。
